...
| `latest_snapshot_ledger`, `reserve_a`, `reserve_b`, `total_shares`, `tvl`, `volume`, `fee_revenue`, `latest_snapshot_at` | DB → latest row from `liquidity_pool_snapshots` within a recent freshness window via LATERAL; NULL when stale |
