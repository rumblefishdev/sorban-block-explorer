...
async fn main() -> Result<(), Error> {
    ...
    let database_url = match std::env::var("DATABASE_URL") {
        Ok(url) => url,
        Err(_) => {
            let secret_arn = std::env::var("DB_SECRET_ARN").map_err(|_| {
                "either DATABASE_URL or DB_SECRET_ARN must be set"
            })?;
            let rds_endpoint = std::env::var("RDS_ENDPOINT").map_err(|_| {
                "RDS_ENDPOINT must be set when using DB_SECRET_ARN"
            })?;
