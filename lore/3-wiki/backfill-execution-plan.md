...
> ([0130](../1-tasks/backlog/0130_BUG_historical-partition-gaps.md),
> [0132](../1-tasks/backlog/0132_FEATURE_missing-db-indexes.md),
> [0134](../1-tasks/backlog/0134_BUG_envelope-meta-ordering-validation.md),
...
| Gate                                            | Task                                                                                                                                                                           | Why it blocks T0                                                                                                                                                                                                                                                                                       |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Monthly partitions exist (locally + on staging) | [0130](../1-tasks/backlog/0130_BUG_historical-partition-gaps.md) (rescope: 7 tables, names per ADR 0037)                                                                       | Without children, every backfill row falls into `_default` — kills partition pruning everywhere. See note in [`backfill-bench/src/main.rs:326`](../../crates/backfill-bench/src/main.rs#L326) — the bench helper currently creates only `_default`, which is why the smoke runs landed entirely there. |
| Envelope/meta ordering validated                | [0134](../1-tasks/backlog/0134_BUG_envelope-meta-ordering-validation.md) (slim AC: passphrase plumbing already shipped via 0160)                                               | Silent corruption blast radius is the entire backfill; cheaper to assert at parse time than diff post-hoc                                                                                                                                                                                              |
| Galexie config + testnet proof                  | [0041](../1-tasks/backlog/0041_FEATURE_galexie-config-testnet-validation.md)                                                                                                   | Cutover at T7 needs Galexie wiring known-good                                                                                                                                                                                                                                                          |
| Indexes (post-cutover, not blocking T0)         | [0132](../1-tasks/backlog/0132_FEATURE_missing-db-indexes.md) (rescope per [`endpoint-queries`](../../docs/architecture/database-schema/endpoint-queries/) INDEX-GAP comments) | Can be added post-restore via `CREATE INDEX CONCURRENTLY` — does not gate the dump                                                                                                                                                                                                                     |
