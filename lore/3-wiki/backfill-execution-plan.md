...
| --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Monthly partitions cover the full Soroban era (local + staging) | [0130](../1-tasks/backlog/0130_FEATURE_historical-partition-gaps.md) — operational; the existing `db-partition-mgmt` Lambda already implements the logic, the work is wiring + invocation | Blocks T0. Without children, every backfill row falls into `_default` — kills partition pruning everywhere. The current local gap is visible in [`backfill-bench/src/main.rs:326`](../../crates/backfill-bench/src/main.rs#L326) — the bench helper creates only `_default`.                               |
